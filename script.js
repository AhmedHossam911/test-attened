...
// Generate a random code based on the current date lec5
{
  const givenCodes = [
    "54S9C21LC7",
    "54S5LC72CE9",
  ...
  const codeElement = document.getElementById('random-code-5');
  if (codeElement) {
    const randomIndex = Math.floor(Math.random() * givenCodes.length);
    codeElement.textContent = givenCodes[randomIndex];
  }
...
// date collect
{
  ...
  if (
    ...
    (currentHour === 20 && currentMinute <= 30)
  ) {
    document.getElementById("Lec5").style.display = "block";
  } else {
...
// day date lec5
{
  const currentDateElement = document.getElementById('currentDate-5');
  const currentDate = new Date();
  const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
  const formattedDate = currentDate.toLocaleDateString('en-US', options);
